...
Matrix createLocalK(int e,mesh &m){
    ...
    float t_var,k_var,delta,lambda,Ae,J,D;
    
    //Componentes de K
    // [ A+K'  G+K'' ]
    // [  D   0 ]
    Matrix matrixA,matrixK,matrixG,matrixD,matrixK2;
    ...
    productRealMatrix(t_var*J/(12*D),productMatrixMatrix(g_matrix,productMatrixMatrix(Alpha,Beta,2,2,6),6,2,6),matrixA);

    //Preparando matrixK (En clase conocida simplemente como K')
    k_var = m.getParameter(K_VAR);
    Ae = calculateLocalArea(e,m);
    transpose(Alpha,Alphat);
    transpose(Beta,Betat);
    productRealMatrix(k_var*Ae/(D*D),productMatrixMatrix(Betat,productMatrixMatrix(Alphat,productMatrixMatrix(Alpha,Beta,2,2,6),2,2,6),6,2,6),matrixK);

    //Preparando matrixG (En clase conocida simplemente como G)
    lambda = m.getParameter(LAMBDA);
    calculateBPrima(BPrima);
    productRealMatrix(2*lambda*J/(6*3*D),productMatrixMatrix(g_matrix,productMatrixMatrix(Alpha,BPrima,2,2,3),6,2,3),matrixG);
    ...
    transpose(Alpha,Alphat);
    transpose(Beta,Betat);
    productRealMatrix(delta*Ae/(8*D*D),productMatrixMatrix(Betat,productMatrixMatrix(Alphat,productMatrixMatrix(Alpha,Beta,2,2,6),2,2,6),6,2,6),matrixK2);
    ...
    ubicarSubMatriz(K,0,5,6,8,sumMatrix(matrixG,matrixK2,6,6));
...
Vector createLocalb(int e,mesh &m){
    ...
    float u_var = m.getParameter(u_var);
    float psi = m.getParameter(PSI);
    float n_var = m.getParameter(N_VAR);
    ...
    b.push_back(J*n_var/6); b.push_back(J*n_var/6); b.push_back(J*n_var/6);
